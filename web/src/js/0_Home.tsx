...
export function Home(props: any) {
    ...
    return <div id='page' className='home'>
        ...
        <p className='home-description paragraph fade-in-2'>
            Travelers beware: The Journey to Mount Sogol is a peculiar one, because the destination is not a physical place. That is not to say it is not <i>real</i>. However, Mount Sogol is to be found on a different plane of existence, not in the mundane world. But how can it be reached? Well...
        </p>
        <p className='home-description paragraph fade-in-3'>
            <i>the door to the invisible must be visible</i>.
        </p>
        <button className='btn fade-in-4' onClick={props.nextStage}>
            BEGIN
        </button>
    </div>;
}
